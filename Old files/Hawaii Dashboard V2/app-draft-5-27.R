...
cvar <- append("None",names(app_df)[-c(1:12,16:128)])
yvar <- names(app_df)[-c(1:12)]
fvar <- append("None",names(app_df)[-c(1:12,16:128)])

ui <- navbarPage(title = div(img(src='imi-pono-title-graphic.png',style="margin-top: -8px; padding-right:10px;padding-bottom:20px", height = 60)),
...
server <- function(input, output) {
    ...
    output$distPlot2 <- renderPlotly({
      if(as.numeric(NROW(unique(app_df[,input$Variable]))) < 10){
        ...
        fig <- fig %>% add_pie(data = percs, labels = ~percs[[1]], values = ~n, domain = list(row = 0, column = 0))
        fig <- fig %>% add_pie(data = percs2, labels = ~percs2[[1]], values = ~n, domain = list(row = 0, column = 1))
        fig <- fig %>% layout(grid=list(rows=1, columns=2),template = 'ggplot2',
